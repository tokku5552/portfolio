...
### Requirement: shadcn/ui is initialized and usable

The repository SHALL include a `components.json` generated by the shadcn/ui CLI, SHALL have at least two shadcn primitives added (for example `Button` and `Card`) under `src/components/ui/`, and SHALL import / compile these primitives as part of the production build. The configuration SHALL use `cssVariables: true` with variable names that do not collide with `brand/tokens.css`.

#### Scenario: shadcn CLI add command succeeds

- **WHEN** a contributor runs `pnpm dlx shadcn@latest add <primitive>`
- **THEN** the primitive is written under `src/components/ui/`, resolves its imports, and type-checks without modification

#### Scenario: Initial shadcn primitives compile in a page

- **WHEN** the production build (`pnpm build`) runs after the initial shadcn primitives have been imported by any component
- **THEN** the build succeeds and the primitives render without console warnings in the browser
...
The change SHALL include an ADR-equivalent record of the following decisions and their rationale: (a) selection of pnpm over yarn / npm / bun, (b) selection of Tailwind CSS + shadcn/ui over styled-components / vanilla-extract / Radix-only, (c) decision to keep Chakra UI coexisting instead of removing it in this change, (d) naming convention for brand CSS variables. The record SHALL live inside the change itself so it is archived alongside the capability specs.
...
- **THEN** the `design.md` Decisions section answers each of (a)-(d) with the chosen option, at least one rejected alternative, and the rationale
