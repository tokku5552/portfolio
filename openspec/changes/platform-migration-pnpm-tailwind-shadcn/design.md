...
- **どの primitive を初期に入れるか**: `Button`, `Card` の 2 個程度。将来の /brand ページで使う想定。本 change では「ビルドが通り `npx shadcn` が動く」ことの確認に留める。
...
5. **Step 4: shadcn/ui 初期化**
   ...
   - `pnpm dlx shadcn@latest add button card` で primitive を 2 個入れて型含めてビルド通過を確認
