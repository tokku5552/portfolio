...
Twilight Blade デザインシステムの Tailwind primitive コンポーネント群（Button / Link / Container / Eyebrow / Orb / GridOverlay）を `src/components/parts/<Name>/` 配下に手書きで提供し、`/` landing と `/articles` などすべてのページから合成可能にする。`class-variance-authority` や `shadcn/ui` を採用せず、`cn()` + brand tokens のみで実装する単一の SSoT。
