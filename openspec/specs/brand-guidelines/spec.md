...
Twilight Blade デザインシステムを `/brand` 公開ページと `brand/` 配下の SSoT として提供し、AI（Claude Design / Claude Code / Cursor / Codex 等の IDE AI、Claude.ai / ChatGPT 等の chat AI、Gamma 等のスライド AI、Midjourney 等の画像 AI）と人間の双方が一意のソースを参照して Twilight Blade に従えるようにする要件群。

`brand/` は最小 SSoT（`tokens.css` + `tokens.ts` + `brand.md`）に絞り、配布は GitHub raw / jsdelivr CDN を介して行う（`public/brand-assets/` ミラーは作らない）。AI rule は `brand/brand.md` 一箇所に集約し、repo root の `CLAUDE.md` からはこれを参照するだけにする。
...
- `brand/tokens.css`: ブランド CSS 変数の真本
- `brand/tokens.ts`: `tokens.css` と同期した TypeScript 型付きトークン
- `brand/brand.md`: AI に paste / URL fetch される single-file spec

これらのファイル以外に CSS 変数 `--color-brand-*` および `--font-brand-*` の値を再定義してはならない（MUST NOT）。`brand/CLAUDE.md` / `brand/README.md` を作成してはならない（MUST NOT）。`public/brand-assets/` ディレクトリを作成してはならない（MUST NOT）。
...
`/brand` ルートは Twitter Card / Open Graph 用の画像として `public/brand-og.png` を `Seo` component 経由で公開しなければならない（MUST）。画像は OGP 表示に適した PNG でなければならない（MUST）：推奨寸法は 1200×630 px、推奨ファイルサイズは < 500 KB だが、同等のアスペクト比（≈ 1.91:1、許容誤差 ±5%）かつ妥当なファイルサイズ（< 2 MB）であれば許容する。`public/brand-assets/` サブディレクトリ配下には配置してはならない（MUST NOT）。
